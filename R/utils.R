...
directories <- function(dataset_name, folderResults){
  ...
  #############################################################################
  folderDendrograms = paste(folderOutput, "/Dendrograms", sep="")
  if(dir.exists(folderDendrograms) == TRUE){
    setwd(folderDendrograms)
    dir_folderDendrograms = dir(folderDendrograms)
    n_folderDendrograms = length(dir_folderDendrograms)
  } else {
    dir.create(folderDendrograms)
    setwd(folderDendrograms)
    dir_folderDendrograms = dir(folderDendrograms)
    n_folderDendrograms = length(dir_folderDendrograms)
  }
  ...
  #############################################################################
  folderBestDendro = paste(folderOutput, "/Best-Dendrogram", sep="")
  if(dir.exists(folderBestDendro) == TRUE){
    setwd(folderBestDendro)
    dir_folderBestDendro = dir(folderBestDendro)
    n_folderBestDendro = length(dir_folderBestDendro)
  } else {
    dir.create(folderBestDendro)
    setwd(folderBestDendro)
    dir_folderBestDendro = dir(folderBestDendro)
    n_folderBestDendro = length(dir_folderBestDendro)
  }
  
  folderSimat = paste(folderResults, "/Similarity-Matrices", sep="")
  if(dir.exists(folderSimat) == TRUE){
    setwd(folderSimat)
    dir_folderSimat = dir(folderSimat)
    n_folderSimat = length(dir_folderSimat)
  } else {
    dir.create(folderSimat)
    setwd(folderSimat)
    dir_folderSimat = dir(folderSimat)
    n_folderSimat = length(dir_folderSimat)
  }
  ...
  # RETURN ALL PATHS                                                          #
  #############################################################################
  retorno$folderSimat = folderSimat
  ...
  retorno$folderBestDendro = folderBestDendro
  retorno$folderDendrograms = folderDendrograms
