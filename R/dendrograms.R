...
###########################################################################
CutreeHClust <- function(parameters){
  ...
  s = 1
  cutreeParalel <- foreach(s = 1:parameters$Number.Folds) %dopar%{
  # while(s<=parameters$Number.Folds){
    ...
    coeficiente = c(0)
    coeficiente.2 = c(0)
    coefHC = data.frame(fold, metodo, coeficiente, coeficiente.2)
    ...
    FolderHClust = paste(parameters$Folders$folderDendrograms, 
                         "/Split-", s, sep="")
    if(dir.exists(FolderHClust)==FALSE){dir.create(FolderHClust)}
    
    #FolderOS = paste(parameters$Folders$folderOutputDataset, "/Split-", s, sep="")
    #if(dir.exists(FolderOS)==FALSE){dir.create(FolderOS)}
    
    FolderSplit = paste(parameters$Folders$folderSimat, "/",
                        parameters$Dataset.Name, "-",
                        parameters$Similarity,
                        "/Split-", s, sep="")
    nome.arquivo = paste(FolderSplit, "/", parameters$Similarity, 
                         ".csv", sep="")
    
    cat("\nOpen matrix correlation\n")
    matrix_correlation = data.frame(read.csv(nome.arquivo))
    rownames(matrix_correlation) <- parameters$NamesLabels
    ...
    for(i in i:length(metodos)){
      ...
      FolderMOS = paste(parameters$Folders$folderDendrograms, "/", 
                        metodos[i], sep="")
      ...
      coeficiente.2 = coef(HC)
      coefHC = rbind(coefHC, data.frame(fold, metodo, 
      ...
      k = 1
      for(k in 1:parameters$Dataset.Info$Labels){
        cat("\ncluster: ", k)   
        ...
        AllPartitions = rbind(AllPartitions, data.frame(num.fold, 
                                                        num.part, 
                                                        num.group,
                                                        names.labels,
                                                        metodo.name))
      ...
      print(system(paste("rm -r ", FolderSMOS, "/Partition-", parameters$Dataset.Info$Labels, sep="")))
  ...
  FM = paste(parameters$Folders$folderDendrograms, "/Methods", sep="")
  if(dir.exists(FM)== FALSE){dir.create(FM)}
  metodos = c("average", "complete", "mcquitty", "single", "ward.D", "ward.D2")    
  a = 1
  while(a<=length(metodos)){
    
    print(system(paste("mv ", parameters$Folders$folderDendrograms, 
          "/", metodos[a], " ", FM, "/", sep="")))
...
###################################################################
bestCoefficient <- function(parameters){
  ...
  FolderSave = paste(parameters$Folders$folderDendrograms, 
                     "/Analysis-Coefs", sep="")
  ...
  while(s<=number_folds){  
    ...
    # enter the fold
    FolderSplit = paste(parameters$Folders$folderDendrograms, 
                        "/Split-", s, sep="")
    ...
    seleciona = filter(coef, coeficiente==1)
    if(nrow(seleciona)==length(metodos)){
      str = "All methods are in first place. You can chose any of them."
      setwd(FolderSave)
      write(str, "best-dendrogram.txt")
      
    } else if(nrow(seleciona)==(length(metodos)-1)){
      str = "\nOnly one method is not in first place. You can chose any of the others."
      setwd(FolderSave)
      write(str, "best-dendrogram.txt")
      
    } else if (nrow(seleciona)==(length(metodos)-2)){
      str = "\nOnly two methods are not in first place. You can chose any of the others."
      setwd(FolderSave)
      write(str, "best-dendrogram.txt")
      
    } else {
      str = "\nPlease, check the documents to verify the coeficients values."
      setwd(FolderSave)
      write(str, "best-dendrogram.txt")
    }
  ...
  write.csv(todos, paste(FolderSave , "/", 
                         dataset_name, "-All-Coefs.csv", sep=""), 
            row.names = FALSE)
  
  write.csv(primeiro, paste(FolderSave , "/", 
                            dataset_name, "-1-Coefs.csv", sep=""), 
            row.names = FALSE)
  
  write.csv(segundo,paste(FolderSave , "/", 
                          dataset_name, "-2-Coefs.csv", sep=""), 
            row.names = FALSE)
  
  write.csv(terceiro, paste(FolderSave , "/", 
                            dataset_name, "-3-Coefs.csv", sep=""), 
            row.names = FALSE)
  
  write.csv(quarto, paste(FolderSave, "/", 
                          dataset_name, "-4-Coefs.csv", sep=""), 
            row.names = FALSE)
  
  write.csv(quinto, paste(FolderSave , "/", 
                          dataset_name, "-5-Coefs.csv", sep=""), 
            row.names = FALSE)
  
  write.csv(sexto, paste(FolderSave , "/", 
  ...
  write.csv(all, paste(FolderSave, "/", 
  ...
  write.csv(final, paste(FolderSave, "/", 
  ...
  gc()
  cat("\n\n##############################")
    cat("\n# END: Coeficient Analysis   #")
    cat("\n##############################\n\n")
}



analisaParticoes <- function(parameters){
  
  metodos = c("average", "complete", "mcquitty", "single", "ward.D", "ward.D2")    
  FolderPart = paste(parameters$Folders$folderDendrograms, 
                     "/Analysis-Partitions", sep="")
  if(dir.exists(FolderPart)==FALSE){dir.create(FolderPart)}
  ...
  while(f<=number_folds){
    ...
    while (p <= num.part) {
      ...
      while (m <= length(metodos)) {
        ...
        # /dev/shm/j-GpositiveGO/Output/Dendrograms/Split-1/average/Clusters
        
        Folder = paste(parameters$Folders$folderDendrograms, 
                       "/Split-", f, "/", metodos[m], 
                       "/Clusters", sep="")
        setwd(Folder)
  ...
  write.csv(final_labels, paste(FolderPart, 
                                "/all-labels.csv", sep=""), 
            row.names = FALSE)
  
  write.csv(final_groups, paste(FolderPart,
                                "/all-groups.csv", sep=""), 
            row.names = FALSE)
  
  write.csv(final_metodos, paste(FolderPart, 
                                 "/all-methods.csv", sep=""), 
            row.names = FALSE)
  ...
  b = 1
  while(b<=parameters$Dataset.Info$Labels){
    #cat("\nB", b)
  ...
  # 
  # cor = "#ed2131"
  # Folder = paste(parameters$Folders$folderPartitions, "/Plots", sep="")
  ...
  # par(mfrow = c(parameters$Dataset.Info$Labels,parameters$Dataset.Info$Labels))
  #   b = 1
  #   while(b<=parameters$Dataset.Info$Labels){
  #     rotulo = rotulos2[b]
...
bestDendrogram <- function(parameters){
  ...
  FolderM = paste(parameters$Folders$folderDendrograms, 
                 "/Methods", sep="")
  
  f = 1
  while(f<=number_folds){
    best_fold = best[f,]
    cat(f, " - ", best_fold$metodo)
    FolderOrigem = paste(FolderM, 
                         "/", best_fold$metodo, 
                         "/Split-", f, "/*", sep="")
    FolderD = paste(parameters$Folders$folderBestDendro, "/Split-", f, sep="")
  ...
  setwd(Folder)
  str = "Save this directory as 'DatasetName.tar.gz' and use it in the next phase."
  write(str, "readme.txt")
  
  retorno$BestMethod = best_fold$metodo
  return(retorno)
